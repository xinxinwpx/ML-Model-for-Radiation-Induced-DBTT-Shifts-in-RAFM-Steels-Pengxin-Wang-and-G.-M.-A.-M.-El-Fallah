...
for name, optimize in models.items():
    ...
    scatter_df.to_excel(f"{name}_scatter.xlsx", index=False)

# Save evaluation metrics
metrics_df = pd.DataFrame(metrics, columns=['Model', 'MSE', 'RMSE', 'MAE', 'R^2'])
metrics_df.to_excel("individual_model_metrics.xlsx", index=False)
